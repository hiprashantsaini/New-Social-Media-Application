...
const createPost = async (req, res) => {
    try {
        ...
        if (planPostCount > 0 && postCount < planPostCount) {
            ...
            // for free version
            if (followersCount <= 200 && postCount < 5) {
                create = true;
...
const getAllPosts = async (req, res) => {
    try {
        ...
        const allPosts = await Post.find({}).sort({ createdAt: -1 })  // Sort by createdAt in descending order
            .populate('author', 'name followers email points')
            .populate('likes', 'name')
...
const addComment = async (req, res) => {
    try {
        const userId = req.userId;//User which doing comment. Not the author of Post.
        const postId = req.params.postId;
...
const likePost = async (req, res) => {
    try {
        const userId = req.userId;//User which doing like. Not the author of Post.
        const postId = req.params.postId;
        const post = await Post.findById({ _id: postId });
        if (post) {
...
const disLikePost = async (req, res) => {
    try {
        const userId = req.userId;//User which doing dislike. Not the author of Post.
        const postId = req.params.postId;//Id of the author of Post.
        const post = await Post.findById({ _id: postId });
